...
const { F_Select, F_Insert, F_Delete } = require("../../model/OrcModel");
...
const fetch_pcrx_file = async (req, res) => {
    try {
        ...
        for (let lData of res_dt.msg) {
            ...
            let values=[lData.receipt_no,lData.agent_trans_no,lData.bank_id,lData.branch_code,lData.agent_code,lData.account_type,lData.product_code,lData.account_number,lData.account_holder_name,lData.deposit_amount,lData.download_flag,lData.collection_by,dateFormat(lData.collected_at, "dd-mmm-yy")];
            let val = `:0, :1, :2, :3, :4,'${dateFormat(lData.transaction_date, "dd-mmm-yy")}', :5, :6, :7, :8, :9, :10,:11,:12`;
            await F_Insert(user_data.bank_id,"TD_COLLECTION", fields, val, values, null, 0)
        }
        // to_date('${dateFormat(lData.transaction_date, "yyyy-mm-dd")}', 'YYYY-MM-DD')
...
const fetchdata_to_server = async (req, res) => {
    try {
        ...
        let fields = "a.brn_cd,a.acc_num,b.cust_name,to_char(a.opening_dt,'yyyy-mm-dd') opening_dt,a.prn_amt,b.phone ",
            ...
            where = `a.CUST_CD = b.CUST_CD AND a.ACC_TYPE_CD = 11 AND nvl(a.acc_status,'O') = 'O' AND   a.constitution_cd !=1 AND a.brn_cd = ${user_data.branch_code} AND a.agent_cd = ${value.agent_code}`,
            order = null,
            flag = 1;
...
module.exports = { fetch_pcrx_file, upload_pctx, upload_pctx_file_data, download_pcrx, download_pcrx_file, create_agent_trans, show_upload_account, fetch_show_account, del_all_pctx_file_data, fetchdata_to_server }