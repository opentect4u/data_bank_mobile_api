...
const search_account = async (req, res) => {
    try {
        ...
        var whrDAta = `bank_id=${value.bank_id} AND branch_code='${value.branch_code}' AND agent_code='${value.agent_code}' ${value.flag ? `AND acc_type = '${value.flag}' AND (customer_name LIKE '%${value.account_number}%' OR account_number LIKE '%${value.account_number}%')` : ''}`,
