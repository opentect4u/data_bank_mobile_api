            ...
            if(agentTrnsData.suc > 0){
                if (agentTrnsData.msg.length > 0){
                    for(let agentTrns of agentTrnsData.msg){
                        const { agent_trns_id, agent_trans_no , bank_id, branch_code, agent_code, received_date } = agentTrns;
                        try{
                            var agentTrnsInsert = await db_Insert('bkp_agent_trans', null, null, null, 0, true, `(SELECT * FROM md_agent_trans a WHERE a.sl_no = ${agent_trns_id} AND a.agent_trans_no = '${agent_trans_no}' AND a.bank_id = ${bank_id} AND a.branch_code = '${branch_code}' AND a.agent_code = '${agent_code}')`)
                            ...
                            if(agentTrnsInsert.suc > 0){
                                ...
                                WriteLogFile(txt);
                                var collectionInsert = await db_Insert('bkp_collection', null, null, null, 0, true, `(SELECT * FROM td_collection a WHERE a.agent_trans_no = '${agent_trans_no}' AND a.bank_id = ${bank_id} AND a.branch_code = '${branch_code}' AND a.agent_code = '${agent_code}' AND a.download_flag = 'Y')`)
